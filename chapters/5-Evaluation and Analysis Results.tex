...
Table \ref{tab:NetatmoCookingCalculations} shows that the calculations varies a lot for each event. Number of packets varies from 403 to 1,082 and bytes from 58,926 to 146,001. The same is found for the baseline calculations, which also varies with 584 packets as the smallest to 972 as the highest amount of packets and for bytes from 77,780 to 132,406 bytes. The biggest packet from both the events and the baseline is mainly 407 bytes, with a few exceptions for both. The average values for both packets, bytes and biggest packet are similar to each other for the events and baseline. Therefore it is not possible to distinguish an event based on the biggest packet. 

Considering the measurement for calculations, the average values, including the standard deviation, for cooking are [652,922] for packets and [88 876, 134 480] for bytes and for the corresponding baseline [650,878] for packets and [87 669, 118 969] for bytes. The values overlaps and therefore it is not possible to distinguish the actual cooking event from the corresponding baseline. This is also visible in the graphs in Figure \ref{fig:NetatmoCookingCalculations} which shows that the calculation of the traffic from the events are similar to the baseline standard traffic. The traffic flows from the graphs, shows that it is possible to compare baseline traffic to event traffic and the calculations are also comparable. 

The same result is further confirmed in the graphs in Figures \ref{fig:NetatmoCookingPackets1} and \ref{fig:NetatmoCookingPackets2} for packets and \ref{fig:NetatmoCookingBytes1} and \ref{fig:NetatmoCookingBytes2} for bytes. For some of the event graphs, changes in traffic pattern are visible during the event time, but that is not applicable for all. The same changes are visible for some of the baseline graphs, so it is not a change that happens every time and only when an event is triggered. The graphs for events in red do not follow a specific pattern compared to the baseline traffic in blue. Therefore, it is not possible to distinguish an event triggered from normal traffic for Netatmo. A cooking event does not show differences graphically or for calculations. 
...
The average value, including the standard deviation, for the cooking event gives the following values [8 300,10 742] in packets and [1 058 927, 1 309 291] in bytes and for the corresponding baseline [7 516,10 324] in packets and [794 873, 1 040 729]. The values for packets overlaps and does not meet the requirement for a significant difference, however they do not overlap for bytes. Since the values for bytes does not overlap, the next step is to see if the calculations are comparable. For the graphs in Figure \ref{fig:MillCookingBytes1} and \ref{fig:MillCookingBytes2}, it is a difference between the traffic before the event and the baseline traffic. As the graphs marked in blue shows, there overall number of bytes is much lower than the graphs marked in red and therefore it is not possible to say that it is the triggered event that is giving the differences in number of bytes.
...
Comparing Table \ref{tab:NedisCookingCalculations} and \ref{tab:NedisBaselineCookingCalculations} shows that overall during the events, more packets and bytes are sent than during standard baseline traffic. While for the biggest packet during the cooking-time, there are not a significant difference to when an event is ongoing. The same is shown in Table \ref{tab:NedisComparingBaselineAndCookingCalculations} where the average for both packets and bytes are higher during events. This is also visible in Figure \ref{fig:NedisCookingCalculations} where all the blue marked bars are lower than the red marked graphs for events with one exception in packets. For the biggest packet sent, the values are around 400 bytes and do not differ from cooking in Table \ref{tab:NedisCookingCalculations} to baseline in \ref{tab:NedisBaselineCookingCalculations}.

The standard deviation values applied to the average values gives the following range for cooking: [23 038, 26 228] for packets and [3 130 392, 3 693 802] for bytes compared to the corresponding baseline: [13 776, 20 272] for packets and [1 597 059, 2 439 023] for bytes. The ranges do not overlap and shows that there is a distinct difference in when a cooking event is ongoing compared to the baseline for both packets and bytes. 

The graphs in Figure \ref{fig:NedisCookingPackets1} and \ref{fig:NedisCookingPackets2} gives the same results, that overall during an event the traffic pattern is different then for the baseline. For Figure \ref{fig:NedisCookingBytes1} and \ref{fig:NedisCookingBytes2} the difference is much clearer than for packets. The graphs for baseline are significantly lower in bytes sent and received to the device than when the event is ongoing. It is therefore necessary to look at the graphs in Figure \ref{fig:NedisCookingPackets1}, \ref{fig:NedisCookingPackets2}, \ref{fig:NedisCookingBytes1} and \ref{fig:NedisCookingBytes2} to see if the differences in calculations are directly connected to the triggered event. However, for both packets and bytes, it is visible that the number of packets sent before the cooking events are not comparable to the standard traffic in the baseline graphs, marked in blue. This means that it is not event triggering that is causing the big changes in calculations and it must be other factors than the event which is causing the visible differences for both calculations and traffic patterns. 
...
Including the average values with the standard deviation gives the following ranges for showering: [669,935] packets and [90 598, 126 960] bytes and corresponding baseline: [588,834] for packets and [77 696, 110 778] for bytes. The ranges for packets and bytes both overlaps and therefore it is not possible to conclude that there are differences in the calculations. 
...
The biggest packet is mainly 456 bytes both during the showering event and the corresponding baseline with some variations of bigger packets sent for both cases. This results in no significant differences during an event. 

The average values including standard deviation are [7 831, 9 433] for packets and [955 807, 1 159 917] for bytes in the cooking event and [6 257, 9 019] for packets and [666 040, 919 866] for bytes. The values for bytes does not overlap and shows a small increase of packets for the events. However, Figures \ref{fig:MillShowerBytes1} and \ref{fig:MillShowerBytes2} shows that the number of bytes before the event is not comparable to the number of bytes to the corresponding baseline graphs and therefore it is not the specific event triggering that is causing the differences in the average values for the bytes. 
...
The average values including standard deviation for Nedis showering is [19 433, 25 693] for packets and [2 327 186, 3 408 448] for bytes and for the corresponding baseline files [9 339, 18 785] for packets and [1 181 163, 2 323 701] for bytes. For both bytes and packets the events have a higher range and they do not overlap. However, looking at the graphs for packets in Figures \ref{fig:NedisShowerPackets1} and \ref{fig:NedisShowerPackets2} and for bytes in Figures \ref{fig:NedisShowerBytes1} and \ref{fig:NedisShowerBytes2} the overall traffic before each event is much higher compared to the traffic flows in the baseline graphs. Therefore, it is not the specific event which is causing the big differences in calculations and they are not comparable as the graphs shows. 
...
The average values with standard deviation added for window open are [40 357, 51 921] for packets and [4 419 627, 6 328 999] for bytes and for the corresponding baseline files [42 860, 54 216] for packets and [4 435 787, 5 649 155] for bytes. These ranges overlaps and does not show any significant difference that can be used to identify that a window is open at the home environment. 
...
Comparing Table \ref{tab:NedisWindowCalculations} for events and \ref{tab:NedisBaselineWindowCalculations} for baseline calculations shows that both cases varies a lot in number of packets and bytes during the traffic capture. For both events and baseline, the number of packets varies from around 50,000 to over 100,000 packets, which is a big difference. The same applies to bytes where both varies from around 7,000,000 to over 15,000,000 bytes. There are no significant differences in the biggest packet during the capture, where all days within both cases are between 400 and 500 bytes. Even though the number of packets and bytes varies much for each event and baseline day, they vary almost equally as the average value shown in Table \ref{tab:NedisComparingBaselineAndWindowCalculations} are almost the same. The graphs in Figure \ref{fig:NedisWindowCalculations} also demonstrates how much the packets vary, but still the average values are very close to each other. 

The average values with standard deviation for window open are [77 230, 116 602] for packets and [8 432 485, 15 102 251] for bytes and for the corresponding baseline files [75 877, 112 767] for packets to [8 945 818, 15 030 532] for bytes. The values are overlapping for both bytes and packets and does not give any indication that a window is open in the home.
...
The average values including standard deviation for a weekend at home are [23 891, 29 403] for packets and [3 236 331, 3 984 115] for bytes and for a weekend gone [20 056, 23 982] for packets and [2 678 543, 3 202 761] for bytes. The ranges for bytes does not overlap and therefore it is expected that the biggest differences graphically should be visible when looking at the graphs in Figure \ref{fig:NetatmoWeekendBytes}. 
...
The average values with standard deviation added for weekends at home are [230 208, 312 618] for packets and [26 582 393, 35 674 435] for bytes, for weekends gone the values are [233 421, 273 869] for packets and [25 269 718, 33 009 800] for bytes. These values overlaps for both packets and bytes and cannot be used to correlate if a user is home or not. 

The graphs in Figure \ref{fig:MillWeekendPackets} and \ref{fig:MillWeekendBytes} shows that the traffic pattern for weekend at home or gone look the same. There are variations in the both weekend at home or gone, but not a constant same variation for all weekends in the same category. This means that is it not possible to distinguish whether a user is home or not by looking at the traffic patterns for Mill.  
...
The calculated average values including standard deviation for weekends at home are [492 503, 592 289] for packets and [57 805 628, 73 137 604] for bytes and for weekends gone [417 808, 661 652] for packets and [51 051 373, 93 071 515]. The value ranges overlaps and can therefore not be used to distinguish if a user is at home or not. 
